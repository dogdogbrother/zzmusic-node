...
class SongCtl {
  ...
  async uploadSong(ctx) {
    ...
    if (common) {
      ...
      if (common.picture && common.picture.length) {  // 如果存在的话，代表歌曲文件有封面
        ...
        resBody.coverPath = `${HOST}:3003/image-store/cover/${uid}${common.picture[0].description || '.jpg'}`
      }
      ctx.body = resBody
    } else {
      ctx.body = {
  ...
  async song(ctx) {
    ...
    if (coverPath) {  // 封面有可能是自己上传的，也有可能是文件自己带的，处理逻辑不一样
      if (coverPath.search(`${HOST}:3003/image-store/cover/`) > -1) { // 已经在cover文件夹里面了，不需要再操作了
    ...
    const coverType = (coverPath) => {
      if (coverPath.search(`${HOST}:3003/image-store/cover/`) > -1) {
        return coverPath
      } else {
        return `${HOST}:3003/image-store/cover/${getFileName(coverPath)}`
      }
    }
    const result = await Song.create({
      songPath: `${HOST}:3003/music-store/${getFileName(songPath)}`,
